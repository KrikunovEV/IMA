...
class Agent:
    ...
    def get_metrics(self):
        return (
            # AvgMetric(self.ema_reward_key, self.cfg.log_avg, epoch_counter=False),
    ...
    def act(self, obs):
        ...
        if self.train:
            logits, logits2, v = self.active_model(state)
        else:
            logits, logits2, v = self.models[self.eval_i](state)
        ...
        prob_active = dist.probs[action] * dist2.probs[action2]
        if prob_active < self.cfg.prob_thr:
            prob_active += self.cfg.prob_thr

        if self.train:
            if self.stage == self.LEARN:
                self.log_p.append(torch.log(prob_active))
                self.entropies.append(dist.entropy())
                self.values.append(v)

            elif self.stage == self.OPE:
                for i in range(len(self.models)):
                    logits, logits2, _ = self.models[i](state)
                    probs = torch.softmax(logits, dim=-1)
                    probs2 = torch.softmax(logits2, dim=-1)
                    prob = probs[action] * probs2[action2]
                    if prob < self.cfg.prob_thr:
                        prob += self.cfg.prob_thr
                    self.ope_values[i].append(prob / prob_active)
        else:
            for i in range(len(self.models)):
                if i == self.eval_i:
    ...
    def _learn(self, next_obs):
        ...
        optimizer.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm_(self.active_model.parameters(), self.cfg.grad_clip)
        optimizer.step()
